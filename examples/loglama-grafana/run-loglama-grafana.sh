#!/bin/bash

# Script to run LogLama with Grafana integration
set -e

echo "=== LogLama + Grafana Integration Setup ==="

# Ensure we're in the right directory
cd "$(dirname "$0")"

# Check if Docker is running
if ! docker info > /dev/null 2>&1; then
    echo "Error: Docker is not running or not accessible"
    exit 1
fi

# Check if docker-compose is available
if ! command -v docker compose &> /dev/null; then
    echo "Error: docker compose is not installed"
    exit 1
fi

# Create logs directory if it doesn't exist
mkdir -p logs

# Ensure we have the fixed files
if [ ! -f "Dockerfile.fixed" ] || [ ! -f "docker-compose.yml.fixed" ]; then
    echo "Error: Fixed configuration files not found"
    exit 1
fi

# Use the fixed files
cp Dockerfile.fixed Dockerfile
cp docker-compose.yml.fixed docker-compose.yml

echo "Using fixed configuration files"

# Build and start the services
echo "Building and starting LogLama and Grafana..."
docker compose down -v
docker compose build --no-cache
docker compose up -d

# Wait for services to be ready
echo "Waiting for services to start..."
sleep 5

# Check if services are running
if docker compose ps | grep -q "loglama.*Up"; then
    echo "LogLama is running"
else
    echo "Warning: LogLama container is not running properly"
fi

if docker compose ps | grep -q "grafana.*Up"; then
    echo "Grafana is running"
else
    echo "Warning: Grafana container is not running properly"
fi

# Show access URLs
echo ""
echo "=== Access Information ==="
echo "LogLama Web UI: http://localhost:5000"
echo "Grafana Dashboard: http://localhost:3000"
echo "  - Default login: admin / admin"
echo ""
echo "To view logs:"
echo "  docker compose logs -f"
echo ""
echo "To stop the services:"
echo "  docker compose down"

# Open browser if xdg-open is available
if command -v xdg-open &> /dev/null; then
    echo "Opening LogLama Web UI in browser..."
    xdg-open http://localhost:5000
    sleep 2
    echo "Opening Grafana Dashboard in browser..."
    xdg-open http://localhost:3000
fi
