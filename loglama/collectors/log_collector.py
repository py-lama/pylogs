...
from loglama.core.env_manager import find_pylama_root, get_project_path
...
COMPONENT_LOG_PATHS = {
    "weblama": {
        "db_path": "logs/weblama.db",
        "log_path": "logs/weblama.log"
    },
    "apilama": {
        "db_path": "logs/apilama.db",
        "log_path": "logs/apilama.log"
    },
    "pybox": {
        "db_path": "logs/pybox.db",
        "log_path": "logs/pybox.log"
    },
    "pyllm": {
        "db_path": "logs/pyllm.db",
        "log_path": "logs/pyllm.log"
    },
    "pylama": {
        "db_path": "logs/pylama.db",
        "log_path": "logs/pylama.log"
    }
}


def get_component_log_paths(component: str) -> Tuple[Optional[Path], Optional[Path]]:
    """
...
def import_logs_from_sqlite(db_path: Path, component: str) -> int:
    ...
    try:
        ...
        source_cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='logs'")
        if not source_cursor.fetchone():
            # Try log_records table instead (used by newer versions)
            source_cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='log_records'")
            if not source_cursor.fetchone():
        ...
        for log in logs:
            ...
            timestamp_field = log_dict.get("timestamp") or log_dict.get("created")
            if isinstance(timestamp_field, str):
                try:
                    ...
                    # Try other formats
                    try:
                        timestamp = datetime.strptime(timestamp_field, "%Y-%m-%d %H:%M:%S,%f")
                    except ValueError:
            ...
            level = log_dict.get("level") or log_dict.get("levelname") or "INFO"
            level_number = log_dict.get("level_number") or log_dict.get("levelno") or 20
            ...
            log_record = LogRecord(
                ...
                function=log_dict.get("function", "") or log_dict.get("funcName", ""),
                line_number=log_dict.get("line_number", 0) or log_dict.get("lineno", 0),
                process_id=log_dict.get("process_id", 0) or log_dict.get("process", 0),
                process_name=log_dict.get("process_name", ""),
                thread_id=log_dict.get("thread_id", 0) or log_dict.get("thread", 0),
                thread_name=log_dict.get("thread_name", ""),
                exception_info=log_dict.get("exception_info", None) or log_dict.get("exc_text", None),
        ...
        logger.info(f"Imported {imported_count} log records from {component} database")
        return imported_count
    
    except Exception as e:
        logger.exception(f"Error importing logs from {component} database: {e}")
        return 0
    ...
    try:
        ...
        for line in log_lines:
            ...
            try:
                ...
                timestamp = log_data.get("timestamp") or log_data.get("created")
                ...
                logger_name = log_data.get("name") or log_data.get("logger_name") or log_data.get("module")
                ...
                if not logger_name and "message" in log_data:
                    ...
                    if " - " in message:
                        ...
                        if "." in potential_logger and not potential_logger.isdigit():
                ...
                level = log_data.get("level") or log_data.get("levelname") or "INFO"
                level_number = log_data.get("level_no") or log_data.get("levelno") or 20
                ...
                log_record = LogRecord(
                    ...
                    function=log_data.get("function", "") or log_data.get("funcName", ""),
                    line_number=log_data.get("line_number", 0) or log_data.get("lineno", 0),
                    process_id=log_data.get("process", 0) or log_data.get("process_id", 0),
                    process_name=log_data.get("process_name", ""),
                    thread_id=log_data.get("thread", 0) or log_data.get("thread_id", 0),
                    thread_name=log_data.get("thread_name", ""),
                    exception_info=log_data.get("exception", None) or log_data.get("exc_text", None),
        ...
        logger.info(f"Imported {imported_count} log records from {component} log file")
        return imported_count
    
    except Exception as e:
        logger.exception(f"Error importing logs from {component} log file: {e}")
        return 0
...
def collect_logs_from_component(component: str) -> int:
    ...
    logger.info(f"Collected {total_count} logs from {component} ({db_count} from database, {file_count} from log file)")
...
if __name__ == "__main__":
    ...
    # Print the results
    print(f"Collected logs from components:")
