...
from loglama.collectors.log_collector import collect_all_logs, collect_logs_from_component
...
def run_collector(components: Optional[List[str]] = None, interval: int = DEFAULT_INTERVAL, 
                 once: bool = False, verbose: bool = False):
    """
    ...
    # Log start
    if components:
        logger.info(f"Starting scheduled log collector for components: {', '.join(components)}")
    else:
    ...
    while running:
        try:
            ...
            if components:
                ...
                logger.info(f"Collected {total_count} logs from specified components")
            else:
            ...
            if sleep_time > 0:
                logger.debug(f"Sleeping for {sleep_time:.2f} seconds until next collection")
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Scheduled Log Collector for LogLama")
    parser.add_argument("--components", "-c", nargs="+", help="Components to collect logs from (default: all)")
    ...
    parser.add_argument("--once", "-o", action="store_true", help="Run only once instead of periodically")
    parser.add_argument("--verbose", "-v", action="store_true", help="Show verbose output")
    
    args = parser.parse_args()
    
    # Run the collector
    run_collector(components=args.components, interval=args.interval, 
                 once=args.once, verbose=args.verbose)
