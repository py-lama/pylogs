    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                log(level, f"Completed {timer_name} in {elapsed:.3f} seconds", 
                    logger_name, operation=timer_name, status="completed", duration=elapsed)
                ...
                # Log error message
                log("error", f"Error in {timer_name} after {elapsed:.3f} seconds: {str(e)}", 
                    logger_name, operation=timer_name, status="error", duration=elapsed, error=str(e))
...
def logged(func=None, *, level: str = "info", logger_name: Optional[str] = None, 
           log_args: bool = True, log_result: bool = True, comment: Optional[str] = None):
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                log("error", f"Error in {func.__name__}: {str(e)}{' - ' + comment if comment else ''}", 
                    logger_name, status="error", error=str(e), **context)
                raise
