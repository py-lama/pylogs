...
try:
    ...
    def set_key(*args, **kwargs):
        logging.warning("python-dotenv package not found, cannot set environment variables")
        return False
...
def find_pylama_root() -> Path:
    ...
    logger.warning(f"Could not find py-lama root, using current directory {Path.cwd()} as fallback")
    return Path.cwd()
    ...
    if TOML_AVAILABLE:
        for subdir in pylama_root.iterdir():
            ...
            try:
                ...
                if project_info.get("name", "").lower() == project_name.lower():
...
def load_central_env(override: bool = True) -> bool:
    ...
    if not DOTENV_AVAILABLE:
        logger.warning("python-dotenv package not found. Install it with 'pip install python-dotenv' for .env file support.")
        return False
    ...
    # Load the .env file
    logger.info(f"Loading environment variables from central .env file: {env_path}")
    success = load_dotenv(env_path, override=override)
    
    return success


def ensure_env_var(key: str, default_value: str = "", description: str = "") -> str:
    """
    ...
    """
    if not DOTENV_AVAILABLE:
        logger.warning("python-dotenv package not found. Cannot ensure environment variables.")
    ...
    for project, required_vars in _required_env_vars.items():
        ...
        for var in required_vars:
            if var not in os.environ:
                ...
                if "LOG_LEVEL" in var:
                    default_value = "INFO"
                elif "LOG_DIR" in var or "OUTPUT_DIR" in var or "SCRIPTS_DIR" in var or "MODELS_DIR" in var:
                    default_value = f"./logs"
                elif "DB_PATH" in var:
...
def check_project_dependencies(project_name: str) -> Tuple[bool, List[str], str]:
    """
    ...
    try:
        ...
        if success:
            ...
            logger.warning(f"Missing dependencies for {project_name}: {missing_deps}")
...
def install_project_dependencies(project_name: str) -> Tuple[bool, str]:
        ...
        if pyproject_file.exists() and TOML_AVAILABLE:
            try:
                ...
                if "tool" in pyproject_data and "poetry" in pyproject_data["tool"]:
                    ...
                    logger.info(f"Installing dependencies for {project_name} using poetry...")
                    process = subprocess.run(
                        ["poetry", "install"],
                        cwd=project_path,
                        capture_output=True,
                    ...
                    if success:
                        logger.info(f"Dependencies for {project_name} installed successfully using poetry")
                    else:
                        logger.warning(f"Failed to install dependencies for {project_name} using poetry")
    ...
    try:
        ...
        if success:
            logger.info(f"Dependencies for {project_name} installed successfully")
        else:
            logger.warning(f"Failed to install dependencies for {project_name}")
...
def start_project(project_name: str, args: List[str] = None) -> Tuple[bool, Any, str]:
    """
    ...
    try:
        ...
        try:
            ...
            if return_code is not None:
                ...
                logger.error(f"{project_name} terminated with code {return_code}: {output}")
