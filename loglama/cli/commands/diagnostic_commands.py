...
@click.command()
@click.argument("command", type=click.Choice(["system", "env", "logs", "deps", "all"]), default="all")
@click.option("--output", default=None, help="Output file for diagnostic information")
...
@click.option("--log-level", default="INFO", help="Log level for diagnostic output")
def diagnose(command, output, log_dir, db_path, log_level):
    ...
    try:
        ...
        if output:
            ...
            console.print(f"[green]Diagnostic results written to {output_path}[/green]")
        else:
...
def version():
    ...
    try:
        ...
        console.print(f"[red]Error showing version information: {str(e)}[/red]")
        sys.exit(1)
