...
from loglama.core.env_manager import (
    ...
    run_project_tests, start_project
)
...
@click.command()
@click.argument("project", type=click.Choice(["loglama", "pylama", "pyllm", "pybox", "weblama"]))
@click.option("--install/--no-install", default=False, help="Install missing dependencies")
@click.option("--verbose/--no-verbose", default=True, help="Show verbose output")
def check_deps(project, install, verbose):
    ...
    try:
        ...
        if not missing_deps:
            console.print(f"[green]All dependencies for {project} are satisfied.[/green]")
            return
        
        # Install dependencies if requested
        if install:
            console.print(f"Installing missing dependencies for {project}...")
            install_project_dependencies(project, missing_deps, verbose=verbose)
            console.print(f"[green]Dependencies for {project} installed successfully.[/green]")
        else:
            console.print(f"[yellow]Missing dependencies for {project}:[/yellow]")
...
def test(project, verbose):
    ...
    try:
        ...
        if success:
            console.print(f"[green]Tests for {project} passed successfully.[/green]")
        else:
...
@click.option("--install-deps/--no-install-deps", default=False, help="Install missing dependencies")
@click.option("--verbose/--no-verbose", default=True, help="Show verbose output")
    ...
    try:
        ...
        if check_deps:
            ...
            for service in services:
                missing_deps = check_project_dependencies(service, verbose=verbose)
                if missing_deps:
            ...
            if all_missing_deps:
                if install_deps:
                    for service, missing_deps in all_missing_deps.items():
                        console.print(f"Installing missing dependencies for {service}...")
                        install_project_dependencies(service, missing_deps, verbose=verbose)
                        console.print(f"[green]Dependencies for {service} installed successfully.[/green]")
                else:
        ...
        for service in services:
            ...
            try:
                ...
                process = start_project(service, [], verbose=verbose, background=True)
                if process:
                    processes[service] = process
                    console.print(f"[green]{service} started successfully (PID: {process.pid})[/green]")
                else:
                    console.print(f"[yellow]Failed to start {service} in background mode.[/yellow]")
            except Exception as e:
            ...
            try:
                ...
                for service, process in processes.items():
                    try:
                        ...
                        console.print(f"[red]Error stopping {service}: {str(e)}[/red]")
        else:
