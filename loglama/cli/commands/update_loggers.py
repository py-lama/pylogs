...
def update_logger_names(db_path: Path, dry_run: bool = False, all_logs: bool = False) -> Tuple[int, int]:
    """
        ...
        for log in logs:
            ...
            if current_logger == "unknown":
                ...
                if prefix != component:
                    ...
                    new_logger = component
            elif "APILama initialized" in message and current_logger != "apilama":
                new_logger = "apilama"
            elif "Docker Container Status" in message and current_logger != "docker":
                new_logger = "docker"
            else:
                # Skip if the logger name is already appropriate
                if current_logger == component or (current_logger.startswith(component) and not current_logger.endswith(".unknown")):
                    continue
                new_logger = component
            ...
            if new_logger != current_logger:
                ...
                if not dry_run:
                    ...
                    logger.info(f"Would update log {log_id}: {current_logger} -> {new_logger}")
                    updated_logs += 1
...
def main(dry_run: bool = False, all_logs: bool = False) -> None:
    ...
    if all_logs:
        ...
        logger.info(f"Found {total_logs} logs with unknown or incorrect logger names")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Update logger names in the LogLama database")
    parser.add_argument("--dry-run", action="store_true", help="Don't actually update the database")
    parser.add_argument("--all", action="store_true", help="Process all logs, not just those with unknown logger names")
