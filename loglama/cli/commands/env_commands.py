...
@click.command()
@click.option("--env-file", default=None, help="Path to .env file to initialize from")
@click.option("--verbose/--no-verbose", default=True, help="Show verbose output")
@click.option("--force/--no-force", default=False, help="Force overwrite of existing .env file")
def init(env_file, verbose, force):
    ...
    try:
        ...
        if verbose:
            console.print(f"Initializing LogLama environment at {central_env_path}")
        
        # Check if central .env file already exists
        if central_env_path.exists() and not force:
            console.print(f"[yellow]Central .env file already exists at {central_env_path}[/yellow]")
        ...
        if env_file:
            ...
            if not env_file_path.exists():
                console.print(f"[red]Error: .env file not found at {env_file_path}[/red]")
                sys.exit(1)
            
            # Copy the file
            with open(env_file_path, "r") as src, open(central_env_path, "w") as dst:
                dst.write(src.read())
            
            if verbose:
                console.print(f"[green]Copied {env_file_path} to {central_env_path}[/green]")
        else:
            ...
            default_env = {
                ...
                "LOGLAMA_DIR": "logs",
                "LOGLAMA_DB_PATH": str(Path.home() / ".loglama" / "loglama.db"),
                "OLLAMA_MODEL": "llama3",
            ...
            if verbose:
                console.print(f"[green]Created default .env file at {central_env_path}[/green]")
        ...
        if verbose:
            console.print("[green]LogLama environment initialized successfully[/green]")
...
@click.command()
@click.option("--verbose/--no-verbose", default=False, help="Show all environment variables, including empty ones")
def env(verbose):
    ...
    try:
        ...
        if not central_env_path.exists():
            console.print(f"[yellow]Central .env file not found at {central_env_path}[/yellow]")
        ...
        if RICH_AVAILABLE:
            ...
            env_table = Table(title=f"Environment Variables from {central_env_path}")
            ...
            for key, value in sorted(os_env_vars.items()):
                if key not in env_vars:
                    ...
                    env_table.add_row(key, value, "[blue]OS environment[/blue]")
        ...
        console.print(f"[red]Error showing environment variables: {str(e)}[/red]")
        sys.exit(1)
