...
API_HOST = get_env("PYLOGS_API_HOST", "127.0.0.1")
API_PORT = get_env("PYLOGS_API_PORT", 9090, as_type=int)
API_DEBUG = get_env("PYLOGS_API_DEBUG", False, as_type=bool)

# Set up logger
logger = get_logger("pylogs.api")
...
def create_app():
    ...
    try:
        ...
        logger.warning("Database module not available. Install pylogs[db] for database support.")
    ...
    def get_logs():
        ...
        try:
            ...
            if "pylogs.db.models" not in sys.modules:
                return jsonify({
                    "status": "error",
                    "message": "Database module not available. Install pylogs[db] for database support."
                }), 500
    ...
    def health_check():
        ...
        return jsonify({
            "status": "success",
            "message": "PyLogs API is running",
            "timestamp": datetime.utcnow().isoformat(),
            "version": get_version()
        })
    
    def get_version():
        """Get the PyLogs version."""
        try:
...
def start_server(host: Optional[str] = None, port: Optional[int] = None, debug: Optional[bool] = None):
    ...
    # Log server start
    logger.info(f"Starting PyLogs API server on {host}:{port} (debug={debug})")
