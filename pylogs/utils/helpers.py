...
from pylogs.handlers import EnhancedRotatingFileHandler, SQLiteHandler
from pylogs.utils.filters import ContextFilter
...
def configure_logging(
    ...
    if file:
        if file_path is None:
            log_dir = os.environ.get("PYLOGS_LOG_DIR", os.path.expanduser("~/.pylogs/logs"))
    ...
    if database:
        if db_path is None:
            db_dir = os.environ.get("PYLOGS_DB_DIR", os.path.expanduser("~/.pylogs/db"))
            os.makedirs(db_dir, exist_ok=True)
            db_path = os.path.join(db_dir, "pylogs.db")
